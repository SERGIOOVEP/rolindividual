...
class Vuelo {
    constructor(id, origen, destino, fecha, hora, horallegada, asientosLibres, precio, usuariosReserva) {
        this.id = id;
        this.origen = origen;
...
function buscarVuelos() {
    ...
    } else  if (pasajerosBusqueda > 10) {
        alert("Sólo puede adquirir un máximo de 10 billetes")//Le añadimos este "else if" para que no pueda comprar más de 10 billetes un usuario.
    } else  {
        encogerBusqueda(); //quito el boton del html para no liarme a la hora de pintar vuelos y que no se lie el navegador al pasar pantalla.
...
function pintarVuelos(vuelosEncontrados) {
    ...
    vuelosEncontrados.forEach(vuelo => {
        ...
        botonCompraVuelo.onclick = function () {
            ...
            //aquí generamos el objeto reserva
